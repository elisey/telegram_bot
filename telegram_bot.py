...
def help(bot, update):
    ...
    update.message.reply_text('Использование:\n/it - Получить смешняшку\n/it 5 - получить пятую смешняшку\nПри повторном чтении смешняшки присылается следующая\nТекущие номера смешняшек запоминаются')
...
def it(bot, update):
    ...
    if len(args) is 1:
        ...
        try:
            quoteIndex = int(args[1])
        except ValueError:
            update.message.reply_text('Неправильный номер смешняшки')
            help(bot, update)
            return
    else:
        update.message.reply_text('Неправильные переметры')
        help(bot, update)
        return
